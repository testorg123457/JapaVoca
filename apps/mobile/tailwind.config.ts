...
import { cssVarColors, gray, mint, radius, spacing, yellow } from './src/theme/tokens';

/**
 * NativeWind(Tailwind) 설정 — Forest Mint 팔레트 토큰.
 *
 * 색은 두 갈래로 제공한다:
 ...
 *  2) raw 팔레트 mint / yellow / gray (모드 무관 고정값) → bg-mint-500 / text-yellow-400 / bg-gray-50 등.
 *
 * ⚠️ brand 는 두 용도가 겹친다: `bg-brand`(semantic, 모드 대응)는 DEFAULT(css var)로,
 *    `bg-brand-500`(raw mint 스케일)은 스케일로 동시에 동작하도록 DEFAULT + mint 스케일을 합쳐 등록한다.
...
const config: Config = {
  ...
  theme: {
    extend: {
      colors: {
        ...cssVarColors,
        // raw 팔레트. brand 는 semantic `bg-brand` 보존을 위해 DEFAULT(css var) + mint 스케일.
        brand: { DEFAULT: 'var(--color-brand)', ...mint },
        mint,
        yellow,
        gray,
      },
