...
import { cssVarColors, gray, radius, spacing, vermilion, yellow } from './src/theme/tokens';

/**
 * NativeWind(Tailwind) 설정 — Vermilion 팔레트 토큰.
 *
 * 색은 두 갈래로 제공한다:
 ...
 *  2) raw 팔레트 vermilion / yellow / gray (모드 무관 고정값) → bg-vermilion-500 / text-yellow-400 / bg-gray-50 등.
 *
 * ⚠️ brand 는 두 용도가 겹친다: `bg-brand`(semantic, 모드 대응)는 DEFAULT(css var)로,
 *    `bg-brand-500`(raw vermilion 스케일)은 스케일로 동시에 동작하도록 DEFAULT + vermilion 스케일을 합쳐 등록한다.
...
const config: Config = {
  ...
  theme: {
    extend: {
      colors: {
        ...cssVarColors,
        // raw 팔레트. brand 는 semantic `bg-brand` 보존을 위해 DEFAULT(css var) + vermilion 스케일.
        brand: { DEFAULT: 'var(--color-brand)', ...vermilion },
        vermilion,
        yellow,
        gray,
      },
